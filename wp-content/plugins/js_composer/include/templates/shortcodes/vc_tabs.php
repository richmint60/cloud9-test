 ...
 */
$title = $interval = $el_class = $icon_render = '';
...
if ( preg_match_all( "/(.?)\[(vc_tab)\b(.*?)(?:(\/))?\]/s", $content, $matches ) ) {
        ...
        for ( $i = 0; $i < count( $matches[ 0 ] ); $i++ ) {
            ...
            if($icon == '') {
                ...
                $tabs_nav .= '<li><a href="#tab-'.$matches[ 3 ][ $i ][ 'tab_id' ].'" data-href="#tab-'.$matches[ 3 ][ $i ][ 'tab_id' ].'"><i class="fa ' . $icon . '"></i>' . $matches[ 3 ][ $i ][ 'title' ] . '</a></li>';
            }
    }
}
$tabs_nav .= '</ul>';
