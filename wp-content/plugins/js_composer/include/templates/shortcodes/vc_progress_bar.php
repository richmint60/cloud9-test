...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
		...
		. '">';
	$output .= '<div class="describe"' . $line['txtcolor'] . '>' . $line['label'] .  '</div>';
	if ( $max_value > 100.00 ) {
		$percentage_value = (float) $line['value'] > 0 && $max_value > 100.00 ? round( (float) $line['value'] / $max_value * 100, 4 ) : 0;
	} else {
		$percentage_value = $line['value'];
	}
	$output .= '<div class="vc_bar ' . esc_attr( implode( ' ', $bar_options ) ) . '" data-percentage-value="' . esc_attr( $percentage_value ) . '" data-value="' . esc_attr( $line['value'] ) . '"' . $line['bgcolor'] . '><span class="vc_label_units" ' . $line['txtcolor'] . '>' . $line['value'] . $units . '</span></div>';
	$output .= '</div>';
}
