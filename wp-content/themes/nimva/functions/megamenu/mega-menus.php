	...
	class MenuWalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if( $depth === 0 && $this->menu_megamenu_status == "enabled" ) {
				$output .= "\n{first_level}\n";
				//$output .= "\n$indent<ul class=\"megamenu-holder\" >\n<ul class='megamenu {megamenu_border}'>\n";
		...
		public function end_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if( $depth === 0  && $this->menu_megamenu_status == "enabled" ) {
				...
				//$output = str_replace( "{first_level}", "<ul class=\"sub-menu columns-".$this->total_num_of_columns. $col_span . " \">", $output );
				$output = str_replace( "{first_level}", "<ul class=\"sub-menu\"><li><ul class=\"megamenu\">", $output );
				if ( $this->total_num_of_columns > $this->max_num_of_columns ) {
					$output = str_replace( "{megamenu_border}","megamenu-border", $output );
				} else {
					$output = str_replace( "{megamenu_border}","", $output );
				}

				foreach($this->submenu_matrix as $row => $columns) {
					$layout_columns = 12 / $columns; //12/4 = 3 
					...
					if( ( $row - 1 ) * $this->max_num_of_columns + $columns < $this->total_num_of_columns ) {
						$output = str_replace( "{row_number_".$row."}", "megamenu-row-columns-" . $columns . " megamenu-border", $output);
					} else {
						$output = str_replace( "{row_number_".$row."}", "megamenu-row-columns-" . $columns, $output);
					}
					$output = str_replace( "{current_row_".$row."}", " columns-".$columns." col-lg-" . $layout_columns . " col-md-" . $layout_columns . " col-sm-" . $layout_columns, $output );
				}
			} else {
				$output .= "$indent</ul>\n";
			}
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if( $depth === 1 && $this->menu_megamenu_status == "enabled" ) {
				...
				if( $this->num_of_columns > $this->max_num_of_columns ) {
					...
					$this->num_of_rows++;
					$output .= "\n</ul>\n<ul class=\"megamenu megamenu-row-".$this->num_of_rows." {row_number_".$this->num_of_rows."}\" {row_width_".$this->num_of_rows."}>\n";
				}
				...
				if( $this->menu_megamenu_title != 'disabled' && $title != "-" && $title != '"-"' ) {
					...
					if ( ! empty( $this->menu_megamenu_thumbnail ) ) {
						$title_enhance = '<span class="megamenu-icon"><img src="' . $this->menu_megamenu_thumbnail . '"></span>';
					} elseif( ! empty( $this->menu_megamenu_icon ) ) {
						$title_enhance = '<span class="megamenu-icon"><i class="' . $this->menu_megamenu_icon. '"></i></span>';
					}
				...
				if( $this->menu_megamenu_widgetarea &&
					...
					$item_output .= '<div class="megamenu-widgets-container second-level-widget">';
					ob_start();
				...
				if ( ! empty( $this->menu_megamenu_thumbnail ) && $this->menu_megamenu_status == "enabled" ) {
					$item_output .= '<a ' . $attributes . '><span class="megamenu-icon"><img src="' . $this->menu_megamenu_thumbnail . '"></span>';
				} elseif( ! empty( $this->menu_megamenu_icon ) && $this->menu_megamenu_status == "enabled" ) {
					$item_output .= '<a ' . $attributes . '><span class="megamenu-icon text-menu-icon"><i class="fa ' . $this->menu_megamenu_icon . '"></i></span>';
				} elseif ( $depth !== 0 && $this->menu_megamenu_status == "enabled") {
					$item_output .= '<a ' . $attributes . '><span class="sf-sub-indicator megamenu"><i class="fa fa-caret-right"></i></span>';
				} else {
		...
		public function display_element( $element, &$children_elements, $max_depth, $depth, $args, &$output ) {
			...
			if (!empty($children_elements[$element->$id_field]) && $element->menu_item_parent == 0) { 
				$element->title =  $element->title . '<span class="sf-sub-indicator"><i class="fa fa-caret-down"></i></span>'; 
				//$element->classes[] = 'sf-with-ul';
			}
			
			if (!empty($children_elements[$element->$id_field]) && $element->menu_item_parent != 0) { 
				$element->title =  $element->title . '<span class="sf-sub-indicator"><i class="fa fa-caret-right"></i></span>'; 
			}
					...
					<div class="fusion-mega-menu-options">
						<p class="field-megamenu-status description description-wide">
							<label for="edit-menu-item-megamenu-status-<?php echo $item_id; ?>">
								<input type="checkbox" id="edit-menu-item-megamenu-status-<?php echo $item_id; ?>" class="widefat code edit-menu-item-megamenu-status" name="menu-item-megamenu-status[<?php echo $item_id; ?>]" value="enabled" <?php checked( $item->fusion_megamenu_status, 'enabled' ); ?> />
								<strong><?php _e( 'Enable Mega Menu', 'Nimva' ); ?></strong>
							</label>
						</p>
                        <!--
						<p class="field-megamenu-width description description-wide">
							<label for="edit-menu-item-megamenu-width-<?php echo $item_id; ?>">
								<input type="checkbox" id="edit-menu-item-megamenu-width-<?php echo $item_id; ?>" class="widefat code edit-menu-item-megamenu-width" name="menu-item-megamenu-width[<?php echo $item_id; ?>]" value="fullwidth" <?php checked( $item->fusion_megamenu_width, 'fullwidth' ); ?> />
								<?php _e( 'Full Width Mega Menu', 'Nimva' ); ?>
							</label>
						</p>
                        -->
						<p class="field-megamenu-columns description description-wide">
							<label for="edit-menu-item-megamenu-columns-<?php echo $item_id; ?>">
								<?php _e( 'Mega Menu Number of Columns', 'Nimva' ); ?>
						...
						<p class="field-megamenu-title description description-wide">
							<label for="edit-menu-item-megamenu-title-<?php echo $item_id; ?>">
								<input type="checkbox" id="edit-menu-item-megamenu-title-<?php echo $item_id; ?>" class="widefat code edit-menu-item-megamenu-title" name="menu-item-megamenu-title[<?php echo $item_id; ?>]" value="disabled" <?php checked( $item->fusion_megamenu_title, 'disabled' ); ?> />
								<?php _e( 'Disable Mega Menu Column Title', 'Nimva' ); ?>
							</label>
						</p>
						<p class="field-megamenu-widgetarea description description-wide">
							<label for="edit-menu-item-megamenu-widgetarea-<?php echo $item_id; ?>">
								<?php _e( 'Mega Menu Widget Area', 'Nimva' ); ?>
								<select id="edit-menu-item-megamenu-widgetarea-<?php echo $item_id; ?>" class="widefat code edit-menu-item-megamenu-widgetarea" name="menu-item-megamenu-widgetarea[<?php echo $item_id; ?>]">
									<option value="0"><?php _e( 'Select Widget Area', 'Nimva' ); ?></option>
									<?php
						...
						<p class="field-megamenu-icon description description-wide">
							<label for="edit-menu-item-megamenu-icon-<?php echo $item_id; ?>">
								<?php _e( 'Mega Menu Icon (use font awesome; e.g: fa-laptop)', 'Nimva' ); ?>
								<input type="text" id="edit-menu-item-megamenu-icon-<?php echo $item_id; ?>" class="widefat code edit-menu-item-megamenu-icon" name="menu-item-megamenu-icon[<?php echo $item_id; ?>]" value="<?php echo $item->fusion_megamenu_icon; ?>" />
							</label>
						</p>
                        <!--
						<a href="#" id="fusion-media-upload-<?php echo $item_id; ?>" class="fusion-open-media button button-primary megamenu-upload-thumbnail"><?php _e( 'Set Thumbnail', 'Nimva' ); ?></a>                        
						<p class="field-megamenu-thumbnail description description-wide">
							<label for="edit-menu-item-megamenu-thumbnail-<?php echo $item_id; ?>">
								<input type="hidden" id="edit-menu-item-megamenu-thumbnail-<?php echo $item_id; ?>" class="fusion-new-media-image widefat code edit-menu-item-megamenu-thumbnail" name="menu-item-megamenu-thumbnail[<?php echo $item_id; ?>]" value="<?php echo $item->fusion_megamenu_thumbnail; ?>" />
								<img src="<?php echo $item->fusion_megamenu_thumbnail; ?>" id="fusion-media-img-<?php echo $item_id; ?>" class="megamenu-thumbnail-image" style="<?php echo ( trim( $item->fusion_megamenu_thumbnail ) ) ? 'display: inline;' : '';?>" />
								<a href="#" id="fusion-media-remove-<?php echo $item_id; ?>" class="remove-megamenu-thumbnail" style="<?php echo ( trim( $item->fusion_megamenu_thumbnail ) ) ? 'display: inline;' : '';?>">Remove Image</a>
							</label>
						</p>
                        -->
